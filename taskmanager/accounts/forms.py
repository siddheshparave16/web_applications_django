    ...
    def clean(self):
        ...
        if username and password and organization_id:
            try:
                ...
                user = TaskManagerUser.objects.filter(
                    username=username, organization=organization
                ).first()

                if not user:
                    raise ValidationError(
                        "Invalid username/organization", code="invalid_login"
                    )
                ...
                if self.user_cache is None:
                    raise ValidationError(
                        "Invalid username/password/organization", code="invalid_login"
                    )
            except Organization.DoesNotExist:
                raise ValidationError(
                    "Invalid organization ID", code="invalid_organization"
                )
        return cleaned_data
...
class CustomUserCreationForm(UserCreationForm):
    ...
    def clean(self):
        ...
        if not organization:
            self.add_error(
                "organization", "Organization selection is required."
            )  # ✅ Fix: Error attached to field
        ...
        if username and organization:
            if TaskManagerUser.objects.filter(
                ...
                self.add_error(
                    "username", "This username is already taken in this organization."
                )
    ...
    def clean_email(self):
        ...
        if email and organization:
            if TaskManagerUser.objects.filter(
                email__iexact=email, organization=organization
            ).exists():
                raise ValidationError(
...
class TaskManagerUserUpdateForm(UserChangeForm):
    ...
    def _validate_unique_field(self, field_name, error_message, error_code):
        ...
        if value:
            if (
                TaskManagerUser.objects.exclude(pk=self.instance.pk)
                .filter(
