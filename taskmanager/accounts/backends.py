...
class OrganizationUsernameOrEmailBackend(ModelBackend):
    def authenticate(self, request, username=None, password = None, organization_id=None, **kwargs):
        ...
        logger.debug(f"Authenticating user: {username}, Organization ID: {organization_id}")
        ...
        try:
            ...
            if not user:
                logger.warning(f"No user found with username/email: {username} and organization_id: {organization_id}")
                return None
